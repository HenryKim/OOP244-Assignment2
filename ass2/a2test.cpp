/* OOP244.132 Assignment 2 Test Main
...
*
*  Version 1.00
*  Originates from Jan 10 2010 (CS)
*  Changes made:
...
*  Assumes that the declaration of the ISBN class is in ISBN.h
*
...
int rTests(ISBNPrefix& list, int& testNumber, int* totalPassed, int* totalTested) {
    ...
    for (int i = 0; i < total_isbn; i++) {
      ...
      // All other ISBN's are valid and registered
        ...
        sprintf(message, "isbn.isRegistered() did not return true  "
            "for %s", value[i]);
        testFailed(message, ++testNumber, &ok, totalTested);

      // Output result status of every 100th ISBN
      } else if (i%100 == 0) {
        sprintf(message, "isbn.isRegistered() returned true  for %s",
         value[i]);
        testPassed(message, ++testNumber, totalPassed, totalTested);

      // Count as a passed test
      } else {
